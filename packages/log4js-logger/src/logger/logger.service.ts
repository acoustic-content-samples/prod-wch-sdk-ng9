...
import { bindMember, isNotEmpty, mapArray } from '@acoustic-content-sdk/utils';
 ...
 *
 * @param aDstPath - root path for the logger files
 *
...
export function createLog4jsLoggerService(aDstPath?: string): LoggerService {
  ...
  if (!LOG4JS_CONFIGURED) {
    ...
    // target dir
    const dstPath = normalize(
      isNotEmpty(aDstPath) ? aDstPath : join(__dirname, '..', '..', 'test')
    );
