...
import {
  DndDraggableConfig,
  DndStateConfig
} from '../services/DndDraggableConfig';
...
export class DndDraggable implements OnInit, OnDestroy {
  ...
  @Output('dndDragStart') public dndDragStart: EventEmitter<
    any
  > = new EventEmitter();
  @Output('dndDragEnd') public dndDragEnd: EventEmitter<
    any
  > = new EventEmitter();
  ...
  @Output('dndCanceled') public dndCanceled: EventEmitter<
    any
  > = new EventEmitter();
  @Output('dndSelected') public dndSelected: EventEmitter<
    any
  > = new EventEmitter();
  ...
  constructor(private element: ElementRef, private dndState: DndState) {
     ...
     */
    (this.nativeElement as any).onselectstart = function(): void {
  ...
  public ngOnInit(): void {
    this.dropSubscription = dropAccepted.subscribe(({ item, list }) => {
      ...
      if (JSON.stringify(this.dndObject) === JSON.stringify(item)) {
        ...
        if (this.dragState) {
          // TODO CLEUE is `effectAllowed` really correct?
          const boxed = cb[this.dragState.effectAllowed];
          if (boxed) {
            (this[boxed] as EventEmitter<any>).emit();
          }
        }
