...
import { DndStateConfig } from '../services/DndDraggableConfig';
import { DndListSettings } from '../services/DndListSettings';
...
export class DndList implements OnInit, OnDestroy {
  ...
  @Output('dndDragOver') public dndDragOver: EventEmitter<
    any
  > = new EventEmitter();
  @Output('dndDrop') public dndDrop: EventEmitter<any> = new EventEmitter();
  @Output('dndInserted') public dndInserted: EventEmitter<
    any
  > = new EventEmitter();
  ...
  public handleDrop(event: DragEvent): boolean {
    ...
    let offset: number =
      this.nativeElement.children.length - 1 - this.dndModel.length;
    if (this.dndDrop) {
      this.invokeCallback(
        this.dndDrop,
        event,
  ...
  public handleDragLeave(event: DragEvent): void {
    ...
    let newTarget: Element = document.elementFromPoint(
      event.clientX,
      event.clientY
    );
   ...
   */
  private getMimeType(types: ReadonlyArray<string>): string {
